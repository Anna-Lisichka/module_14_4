...
async def get_buying_list(message):
    ...
    for i in Products:
        ...
        with open(f'files/{n}.jpeg', 'rb') as img:
            await message.answer_photo(img, f'Название: Название: {i[0]} | Описание: Описание {i[1]} | Цена: {i[2]}')
            n = n + 1
...
async def get_formulas(call):
    await call.message.answer(
        f'Если Вы мужчина, то формула расчета такая: 10 х вес(кг) + 6,25 х рост(см) - 5 х возраст(г) + 5')
    await call.message.answer(
        f'Если Вы женщина, то формула расчета такая: 10 х вес(кг) + 6,25 х рост(см) - 5 х возраст(г) - 161')
    await call.answer()
